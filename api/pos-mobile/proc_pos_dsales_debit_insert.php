...
// Validasi khusus debit
if (empty($debitamount) || $debitamount == "0") {
    echo json_encode([
        "status" => "ERROR",
        "message" => "Jumlah debit tidak boleh 0"
    ]);
    exit;
}
...
if (empty($cardno)) {
    echo json_encode([
        "status" => "ERROR",
        "message" => "Nomor kartu wajib diisi"
    ]);
    exit;
}
...
$debitamount = sanitizeNumber($debitamount);
$paygiven = sanitizeNumber($paygiven);
$donasiamount = sanitizeNumber($donasiamount);
$point = sanitizeNumber($point);

try {
    // Panggil function proc_pos_dsales_debit_insert
    $sql = "SELECT * FROM proc_pos_dsales_debit_insert(
        :p_ad_mclient_key,
        :p_ad_morg_key,
        :p_serialno,
        :p_pos_mcashier_key,
    ...
    $stmt->bindParam(":p_debitamount", $debitamount);
    ...
    if ($o_message == "success") {
        echo json_encode([
            "status" => "SUCCESS",
            "message" => "Pembayaran Debit berhasil",
            "data" => [
                "new_billno" => $o_data,
                "old_billno" => $billno,
        ...
        echo json_encode([
            "status" => "ERROR",
            "message" => $o_message ?: "Gagal memproses pembayaran Debit"
        ]);
    }
